...
#   y = round(y, digit)
#   try:
#     result = round(x + math.log(round(1 + math.exp(y-x), digit)), digit) # exp(±710)くらいでエラーが発生
...
def EM(df, pi_original, a_a_original, a_b_original, b_original, state_set, label_set, L, epsilon):
  ...
  while True:
    ...
    mu_aa_t = mu_aa_t.astype('int64')

    mu_ab_t = np.zeros((len(state_set), len(state_set)))
    mu_ab_t = mu_ab_t.astype('int64')
    ...
    mu_b_t = mu_b_t.astype('int64')

    mu_pi_t = np.zeros(len(state_set))
    mu_pi_t = mu_pi_t.astype('int64')
    ...
    # print(b_em)
    # print(likelihoods)
    L_sum = (sum(likelihoods)) / (10**digit)
    L_all.append(L_sum) # 対数（likelihood）の足し算は掛け算
