...
def parse_glycan(glycan, state_set, pi, a_a, a_b, b):
    ...
    for node in reversed(glycan):
        for state in state_set:
            # print(node.no)
            phi_up.at[state, node.no] = Decimal(str(float(calc_phi_up(state, node, phi_back, a_a, b, state_set))))
            phi_back.at[state, node.no] = Decimal(str(float(calc_phi_back(state, node, phi_up, phi_back, a_b, state_set))))
...
def calc_phi_up(q, p, phi_back, a_a, b, state_set):
  #calculate phi up
  if p.child == None:
    return copy.deepcopy(b.at[q, p.name])
  else:
    prob = []
    for m in state_set:
      if phi_back.at[m, p.child.no] == 0:
        print("ok0") # 計算していないphi_backを使っている状態
      # Σの部分をmaxに
      # j is the p's eldest children (thus, j == p.child)
      prob.append(Decimal(str(float(a_a[q][m]))) + Decimal(str(float(phi_back.at[m, p.child.no])))) # 対数なので足し算は掛け算
    return Decimal(str(float(b.at[q, p.name]))) + Decimal(str(float(max(prob))))
...
def calc_phi_back(m, j, phi_up, phi_back, a_b, state_set):
  ...
  if j.younger == None: # youngest
    ...
    return copy.deepcopy(phi_up.at[m, j.no])
  else:
    prob = []
    for l in state_set:
      # Σの部分をmaxに
      prob.append(Decimal(str(float(a_b[m][l]))) + Decimal(str(float(phi_back.at[l, j.younger.no])))) # 対数なので足し算は掛け算
    return Decimal(str(float(phi_up.at[m, j.no]))) + Decimal(str(float(max(prob))))

def psi_up(q, p, phi_back, a_a, state_set):
  if p.child == None: # ψ_upにおいてpは必ず子供を持つのでこの条件は該当しないはず
    print("ERROR! P shoud not have a child!")
  else:
    prob = np.empty(0)
    for m in state_set: # m=0to9
      if phi_back.at[m, p.child.no] == 0:
        print("Unexpected value") # 計算していないbackを使っている状態
      # Σの部分をargmaxに
      # j is the p's eldest children (thus, j == p.child)
...
def psi_back(m, j, phi_back, a_b, state_set):
  if j.younger == None: # ψ_backにおいてjは必ずyoungerを持つのでこの条件は該当しないはず
    ...
    prob = np.empty(0)
    for l in state_set: # l=0to9
      # Σの部分をargmaxに
      prob = np.insert(prob, l, a_b[m][l] + phi_back.at[l, j.younger.no]) # 対数なので足し算は掛け算
    return np.argmax(prob)

def most_likely_prob(pi, phi_up, state_set):
  likely = []
  for l in state_set:
    likely.append(Decimal(str(float(pi[l]))) + Decimal(str(float(phi_up.at[l, 0]))))
...
def most_likely_state(glycan, state_set, sugar_id, pi, a_a, a_b, b, phi_up, phi_back):
  ...
  for node in glycan:
    # print(node.no)
    if node.no == sugar_id[-1]:
      prob = np.empty(0)
      for l in state_set:
        prob = np.insert(prob, l, pi[l] + phi_up.at[l, node.no])
      z[node.no] = np.argmax(prob)
