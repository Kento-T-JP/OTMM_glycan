...
def smoothmax(x, y):
  ...
  y = round(y, round_no)
  try:
    return x + round(math.log(1 + round(math.exp(y-x), round_no)), round_no) # exp(±710)くらいでエラーが発生
  except: 
    # logsumexpと同じ処理
...
def EM(df, pi_original, a_a_original, a_b_original, b_original, state_set, label_set, L, epsilon):
  ...
  while True:
    ...
    for row in df.itertuples():
      ...
      forward = forward.round(round_no)
      down = down.round(round_no)
      ...
      for q in state_set:
        for l in state_set:
          ...
          if exist_child == True or total != 0: # 本来total×L_uなのでtotal=0の時は0（対数変換によって掛け算が足し算になっている）
            mu_aa_u[q][l] = round(total - L_u, round_no) # 対数の引き算は割り算
          else:
      ...
      for q in state_set:
        for l in state_set:
          ...
          if exist_younger == True or total != 0:
            mu_ab_u[q][l] = round(total - L_u, round_no) # 対数の引き算は割り算
          else:
            mu_ab_u[q][l] = 0
      ...
      for m in state_set:
        for o_h in label_set:
          ...
          if exist_oh == True or total != 0:
            mu_b_u.at[m, o_h] = round(total - L_u, round_no) # 対数の引き算は割り算
          else:
    ...
    for q in state_set:
      ...
      for l in state_set:
        ...
        a_a[q][l] = round(numerator - denominator, round_no) # 対数の引き算は割り算
    ...
    for q in state_set:
      ...
      for l in state_set:
        ...
        a_b[q][l] = round(numerator - denominator, round_no)
    ...
    for m in state_set:
      ...
      for o_h in label_set:
        ...
        b.at[m, o_h] = round(numerator - denominator, round_no)
    ...
    for m in state_set:
      ...
      pi[m] = round(numerator - denominator, round_no)
